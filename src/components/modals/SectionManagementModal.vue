<template>
  <b-modal id="bv-modal-example" size="lg" title="Gestión de secciones" centered scrollable @hide="closeButton" no-close-on-backdrop>
    <div v-if="!loading && !closing">
      ...
      <table id="customers">
        ...
        <tr v-for="(section, index) in sectionsData" :key="index">
          ...
          <td>
            <!--Botón de subir-->
            <b-button variant="outline-secondary" v-b-tooltip.hover title="Subir" style="font-size: 14px;" size="sm" v-if="index!==0&&!busy" @click="sectionUp(index)">
              <b-icon icon="chevron-up" class="marginLeftButtonSelected"/>
            </b-button>
            <b-button variant="outline-secondary" v-b-tooltip.hover title="Subir" style="font-size: 14px;" size="sm" v-else-if="index==0||busy" disabled @click="sectionUp(index)">
              <b-icon icon="chevron-up" class="marginLeftButtonSelected"/>
            </b-button>
            <!--Botón de bajar-->
            <b-button variant="outline-secondary" v-b-tooltip.hover title="Bajar" style="font-size: 14px;" size="sm" v-if="index!==sectionsData.length-1&&!busy" @click="sectionDown(index)">
              <b-icon icon="chevron-down" class="marginLeftButtonSelected"/>
            </b-button>
            <b-button variant="outline-secondary" v-b-tooltip.hover title="Bajar" style="font-size: 14px;" size="sm" v-else-if="index==sectionsData.length-1||busy" disabled @click="sectionDown(index)">
              <b-icon icon="chevron-down" class="marginLeftButtonSelected"/>
            </b-button>
            <!--Botón de clonar-->
            <b-button variant="outline-secondary" v-b-tooltip.hover title="Clonar" style="font-size: 14px;" size="sm" v-if="!busy" @click="sectionClone(index)">
              <b-icon icon="layers" class="marginLeftButtonSelected"/>
            </b-button>
            <b-button variant="outline-secondary" v-b-tooltip.hover title="Clonar" style="font-size: 14px;" size="sm" v-else-if="busy" disabled @click="sectionClone(index)">
              <b-icon icon="layers" class="marginLeftButtonSelected"/>
            </b-button>
            <!--Botón de eliminar-->
            <b-button variant="outline-danger" v-b-tooltip.hover title="Eliminar" style="font-size: 14px;" size="sm" v-if="sectionsData.length>1&&!busy" @click="sectionDelete(index)">
              <b-icon icon="trash-fill" class="marginLeftButtonSelected"/>
            </b-button>
            <b-button variant="outline-danger" v-b-tooltip.hover title="Eliminar" style="font-size: 14px;" size="sm" v-else-if="sectionsData.length==1||busy" disabled @click="sectionDelete(index)">
              <b-icon icon="trash-fill" class="marginLeftButtonSelected"/>
            </b-button>
          </td>
        </tr>
    ...
    <template v-slot:modal-footer>
      <b-button style="font-size: 16px;" class="size" size="mt-1" variant="outline-secondary" @click="hide">Guardar y cerrar</b-button>
    </template>
  </b-modal>
</template>
